export const Bio = {
  name: "Aayush Shrestha",
  roles: [
    "Full Stack Developer",
    "Programmer",
  ],
  description:"Dedicated and driven Computer Engineering student with a passion for technology and problem-solving. Quick learner with a strong adaptability to new technologies and programming languages. ",
  github: "https://github.com/aayush105",
  resume:
    "https://drive.google.com/file/d/1-KOxra-1C-i1vWI-01RfiQfGoaBdSxUl/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/aayushrestha/",
  twitter: "https://twitter.com/aayushrestha5",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Java",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg",
      },
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },   
    ],
  },
  // {
  //   title: "Android",
  //   skills: [
  //     {
  //       name: "Java",
  //       image:
  //         "https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg",
  //     },
  //     {
  //       name: "Kotlin",
  //       image:
  //         "https://www.vectorlogo.zone/logos/kotlinlang/kotlinlang-icon.svg",
  //     },
  //     {
  //       name: "XML",
  //       image:
  //         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSBMw6_RdwKQ9bDFfnKDX1iwMl4bVJEvd9PP53XuIw&s",
  //     },
  //     {
  //       name: "Android Studio",
  //       image:
  //         "https://developer.android.com/static/studio/images/new-studio-logo-1_1920.png",
  //     },
  //   ],
  // },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://camo.githubusercontent.com/fbfcb9e3dc648adc93bef37c718db16c52f617ad055a26de6dc3c21865c3321d/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6769742d73636d2f6769742d73636d2d69636f6e2e737667",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Netlify",
        image:
          "https://seeklogo.com/images/N/netlify-logo-BD8F8A77E2-seeklogo.com.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Postman",
        image:
          "https://camo.githubusercontent.com/93b32389bf746009ca2370de7fe06c3b5146f4c99d99df65994f9ced0ba41685/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f676574706f73746d616e2f676574706f73746d616e2d69636f6e2e737667",
      },
      {
        name: "Vercel",
        iamge: "data:image/png;base64,",
      }
    ],
  },
];

// export const experiences = [
//   {
//     id: 0,
//     img: "xyz",
//     role: "xyz",
//     company: "xyz",
//     date: "June 2023 - Present",
//     desc: "xyz",
//     skills: [
//       "xyz",
//       "xyz",
//     ],
//     doc: "xyz",
//   },
// ];

export const participations = [
  {
    id: 0,
    title: "KEC LITE 2024",
    role: "Project Management Committee",
    desc: "Led project management, overseeing the submission and approval process for student projects. Evaluated project proposals to ensure alignment with academic objectives and feasibility within project constraints.",
  },
  {
    id: 1,
    title: "KEC-Hack-a-Lite",
    role: "Organizer",
    desc: "Organized KEC-Hack-a-Lite, a 2-day hackathon event held on college premises. Selected and facilitated participation for 12 teams, overseeing all aspects of event planning and execution.",
  },
  {
    id: 2,
    title: "KEC Tech Fest",
    role: "Volunteer",
    desc: "Served as a volunteer at KEC Tech Fest, assisting with event setup, registration, and participant guidance. Provided technical support and troubleshooting for participants during the event.",
  },
  {
    id: 3,
    title: "Kathmandu Youth Conclave 2019",
    role: "Volunteer and Photographer",
    desc: "Engaged as a volunteer and photographer at the Kathmandu Youth Conclave 2019, a significant national event promoting youth leadership and networking. Contributed to creating connections among youth from diverse sectors and capturing moments of our country&apos;s aspiring future leaders."
  }
]
export const education = [
  {
    id: 0,
    img: "https://kec.edu.np/wp-content/uploads/2019/09/KEC_logo.png",
    school: "kantipur Engineering College, Dhapakhel,Lalitpur",
    date: "2020 - Present",
    grade: "Recent Percentage: 80.96%",
    desc: "I am currently pursuing a Bachelor&apos;s degree in Computer Engineering at kantipur Engineering College, Dhapakhel,Lalitpur.",
    degree: "Bachelor in Computer Engineering",
  },
  {
    id: 1,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRJ8Fi8xm5wU_n48j2epPVk21O1YeKjdnx0iQpdJDc3FQ&s",
    school: "Kathmandu Model College, Balkumari, Lalitpur",
    date: "2018 - 2019",
    grade: "GPA: 3.21/4",
    desc: "I completed my high school education at Kathmandu Model College, Balkumari, Lalitpur.",
    degree: "National Examinations Board (NEB) (+2)",
  },
  {
    id: 2,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT9Qqd7Cqfc8SCSunOeVHyPVs2sTCgT8fEIG-JEofRdGg&s",
    school: "Nexus International Academy, Pepsicola, Kathmandu",
    date: "2014 - 2017",
    grade: "GPA: 3.25/4",
    desc: "I completed my class 10 education at Nexus International Academy, Pepsicola.",
    degree: "Secondary Education Examination (SEE)",
  },
];

export const projects = [
  {
    id: 0,
    title: "HAND DRAWN CIRCUIT TO DIGITAL CIRCUIT",
    // image: "",
    description: "Leading an ongoing major project focused on converting hand-drawn circuits into digital circuits. Using image processing and circuit analysis techniques, the aim is to accurately recreate circuit layouts digitally. This initiative simplifies circuit design by employing software tools to replicate and test circuits for functionality and precision.",
    tags: [
      "Python",
      "Streamlit",
      "Ultralytics",
      "Opencv",
      "NumPy",
      "YOLO",
      "Schemdraw",
      "PySpice",
    ],
    category: "web app",
    github: "https://github.com/H2DC",
    // webapp: "",
    member: [
      {
        name: "Aayush Shrestha",
        img: "https://avatars.githubusercontent.com/u/115408327?v=4",
        linkedin: "https://www.linkedin.com/in/aayushrestha/",
        github: "https://github.com/aayush105",
      },
      {
        name: "Anuj Shakya",
        img: "https://avatars.githubusercontent.com/u/63465509?v=4",
        // linkedin: "",
        github: "https://github.com/Anujshakya",
      },
      {
        name: "Jasmine Bajracharya",
        img: "https://avatars.githubusercontent.com/u/63485374?v=4",
        // linkedin: "",
        github: "https://github.com/Jasmineb1",
      },
      {
        name: "Ojashwi Neupane",
        // img: "",
        // linkedin: "",
        github: "https://github.com/Ojashwinpn",
      },
    ],
  },
  {
    id: 1,
    title: "IOE RESULT AND NOTICE VIEWER WEBSITE",
    // image: "",
    description: "IOE Result and Notice Viewer is a website that provides result status, notice viewing, subscribing and query/contact features. The results are sent automatically via email to the registered users, allows the user to view the notices published by IOE. It utilizes Python, Django, SQL, OCR and automation for email notifications.",
    tags: [
      "Python",
      "Sql",
      "OCR",
      "Email Automation",
      "Web scarpping",
      "Django",
      "HTML",
      "CSS",
      "Bootstrap",
    ],
    category: "web app",
    github: "https://github.com/aayush105/Kec-website",
    // webapp: "",
    member: [
      {
        name: "Aayush Shrestha",
        img: "https://avatars.githubusercontent.com/u/115408327?v=4",
        linkedin: "https://www.linkedin.com/in/aayushrestha/",
        github: "https://github.com/aayush105",
      },
      {
        name: "Aliz Shrestha",
        // img: "",
        // linkedin: "",
        // github: "",
      },
      {
        name: "Ankit Kafle",
        // img: "",
        // linkedin: "",
        // github: "",
      },
      {
        name: "Kahsyap Ghimire",
        // img: "",
        // linkedin: "",
        // github: "",
      },
    ],
  },
  {
    id: 2,
    title: "BLOG WEBSITE",
    // image: "",
    description: "The blog website uses MERN (MongoDB, Express.js, React.js, and Node.js) to create a platform where you can add daily blog posts. These posts are stored in a connected database, making it easy to manage and access your blogs.",
    tags: [
      "React Js",
      "MongoDb",
      "Node Js",
      "Express Js",   
    ],
    category: "web app",
    github: "https://github.com/aayush105/Blog-website",
    // webapp: "",
  },
  {
    id: 3,
    title: "BOOK COURSE APPLICATION",
    // image: "",
    description: "Book Course uses Spring Boot (backend) and React.js (frontend) with SQL for database. It lets you add, edit, and delete books. The backend&apos;s API connects smoothly to the frontend via Axios for easy communication. The app offers a straightforward interface, employing Reactstrap for responsiveness and smooth navigation.",
    tags: [
      "Spring Boot",
      "React Js",
      "SQL",
      "Axios",
      "Reactstrap",
      "API",
    ],
    category: "web app",
    github: ["https://github.com/aayush105/book-course-backend","https://github.com/aayush105/book-course-web"],
    // webapp: "",
  },
  {
    id: 4,
    title: "SUPERMARKET BILLING SYSTEM",
    image: "https://github.com/aayush105/supermarket-billing-system/raw/master/Admin.png",
    description: "Developed a Java-based Supermarket Billing System using Swing, enabling easy management of supermarket tasks like billing, sales, transactions, and stock details. Designed separate interfaces for Admin and Cashier roles to streamline their specific responsibilities for efficient operation.",
    tags: [
      "Java",
      "Swing",
      "Netbeans",
      "SQL",
      "JDBC",
      "Thymeleaf",
    ],
    category: "web app",
    github: "https://github.com/aayush105/supermarket-billing-system",
    // webapp: "",
  },

  // format 

  // {
  //   id: 0,
  //   title: "HAND DRAWN CIRCUIT TO DIGITAL CIRCUIT",
  //   description: "",
  //   tags: [],
  //   category: "web app",
  //   github: "",
  //   webapp: "",
  //   member: [
  //     {
  //       name: "",
  //       img: "",
  //       linkedin: "",
  //       github: "",
  //     }
  //   ],
  // },
];

export const certifications = [
  {
    id: 0,
    title: "Java Package",
    date: "Dec 2023",
    org: "Broadway Infosys",
    desc: "Completed a comprehensive Java Package course at Broadway Infosys. Developed practical skills in both Core and Advanced Java concepts, gaining proficiency in desktop and web-based application development. Covered topics such as Servlets, JSP, Swing, MVC, Hibernate, Spring Framework, and more.",
    // link: "",
  },
  {
    id: 1,
    title: "Postman API Fundamentals course",
    date: "2023",
    org: "Postman",
    desc: "Learned API concepts, testing, automation, monitoring and collaboration with Postman.",
    // link: "",
  },
  {
    id: 2,
    title: "Android Application Development",
    date: "2022",
    org: "Kantipur Engineering College",
    desc:"Completed a comprehensive 60-hour training on Android Application Development at Kantipur Engineering College, facilitated by the Research Training and Consultancy Division (RTCD).",
    // link: "",
  },
]

export const achievements = [
  {
    id: 0,
    title: "Academic Scholarships",
    desc: "Awarded scholarship from Kantipur Engineering College for being one of the rankers in board examinations.",
  },
  {
    id: 1,
    title: "Frogtoberfest",
    desc: "Participated in the Open Source Hackathon hosted by Leapfrog Technology, Inc. for Leapfroggers and LeapNetwork. Recognized as the top contributor, contributing significantly to open-source.",
  },
  {
    id: 2,
    title: "60 Days Learning Challenge",
    desc: "Engaged in the 60 Days Learning Challenge by Leapfrog Technology, Inc. where I committed to learning something new every day, receiving swags upon successful completion of the challenge.",
  },
  {
    id: 3,
    title: "KMC-SRCC Collaborative Program",
    desc: "Honored with a token of appreciation for participating and contributing effectively to the joint program between KMC and Sri Ram College of Commerce (SRCC). Engaged in a competition that promoted teamwork and learning, focusing on case studies and business models.",
  },
]
